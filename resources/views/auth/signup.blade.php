...
<body>
    <form method="POST" action="{{ route('signup') }}">
        ...
        <div>
            <label for="name">{{ __('名前 (Họ tên)') }}</label>
            <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus>
        </div>

        <div>
            <label for="email">{{ __('メール (E-Mail)') }}</label>
        ...
        <div>
            <label for="password_confirmation">{{ __('パスワードを確認する (Nhập lại mật khẩu)') }}</label>
            <input id="password_confirmation" type="password" name="password_confirmation" required>
        </div>
        <!-- Add role -->
        <div>
            <label for="role">{{ __('あなたは (Bạn là)') }}</label>
